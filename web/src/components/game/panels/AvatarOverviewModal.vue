...
const tabs = [
  { key: 'overview', labelKey: 'game.avatar_overview.tabs.overview' },
  { key: 'deceased', labelKey: 'game.avatar_overview.tabs.deceased' },
]
...
<template>
  <m-dialog :show="show" @update:show="handleShowChange" :title="t('game.avatar_overview.title')"
    style="width: 980px; max-height: 80vh; overflow-y: auto;">
    <m-loading :show="avatarOverviewStore.isLoading">
      <div class="avatar-overview" :style="panelStyleVars">
        ...
        <div class="tabs-header">
          <button v-for="tab in tabs" :key="tab.key" class="tab-btn" :class="{ active: activeTab === tab.key }"
            type="button" @click="handleTabUpdate(tab.key)">
            {{ t(tab.labelKey) }}
          </button>
        </div>
        ...
        <div v-show="activeTab === 'overview'" class="tab-content">
          ...
          <section class="section">
            <div class="section-title">
              <span class="section-title-icon" :style="{ '--icon-url': `url(${trophyIcon})` }"
        ...
        <!-- Deceased Tab -->
        <div v-show="activeTab === 'deceased'" class="tab-content">
          <template v-if="selectedRecord">
            ...
            <div class="deceased-detail-card">
              ...
              <div class="detail-row">
                ...
                <span>{{ t('game.deceased.death_year', {
                  year: selectedRecord.deathYear, month:
                    selectedRecord.deathMonth }) }}</span>
              </div>
          ...
          <template v-else>
            <m-loading :show="deceasedLoading">
              <div v-if="!deceasedLoading && records.length === 0" class="empty-state">
