...
def apply_runtime_content_locale(lang_code: str) -> None:
    ...
    _apply_runtime_content_locale(
        game_instance=room_registry.get_runtime().state,
...
manager = ConnectionManager(runtime=room_registry, is_idle_shutdown_enabled=is_idle_shutdown_enabled)
...
def build_update_init_progress(target_runtime):
    def _update(phase: int, phase_name: str = ""):
        _update_init_progress(runtime=target_runtime, phase=phase, phase_name=phase_name)

    return _update


def build_init_game_async(target_runtime):
    async def _init_game_async():
        await perform_game_initialization(
            runtime=target_runtime,
            avatar_assets=AVATAR_ASSETS,
            assets_path=ASSETS_PATH,
            config=CONFIG,
            update_init_progress=build_update_init_progress(target_runtime),
...
async def init_game_async():
    """兼容保留：在主房间异步初始化游戏世界。"""
    await build_init_game_async(runtime)()
...
async def game_loop():
    ...
    await run_game_loop_forever(
        get_runtime_contexts=room_registry.iter_rooms,
        manager=manager,
        build_avatar_updates=lambda world: build_avatar_updates(
            world=world,
        ...
        build_tick_state=lambda avatar_updates, events, world, room_id: build_tick_state(
            room_id=room_id,
            world=world,
            events=events,
...
command_handlers = create_command_handlers(
    ...
    get_init_game_async=lambda target_runtime: build_init_game_async(target_runtime),
    ...
    grant_player_support_for_avatar=grant_player_support_for_avatar,
    appoint_player_seed_for_avatar=appoint_player_seed_for_avatar,
    set_player_directive_for_sect=set_player_directive_for_sect,
    clear_player_directive_for_sect=clear_player_directive_for_sect,
    intervene_relation_for_sects=intervene_relation_for_sects,
...
run_grant_avatar_support = command_handlers.run_grant_avatar_support
...
run_update_world_room_entitlement = command_handlers.run_update_world_room_entitlement
run_create_world_room_payment_order = command_handlers.run_create_world_room_payment_order
run_settle_world_room_payment = command_handlers.run_settle_world_room_payment
run_receive_sepay_world_room_payment_webhook = command_handlers.run_receive_sepay_world_room_payment_webhook
run_reconcile_world_room_payment = command_handlers.run_reconcile_world_room_payment
run_add_world_room_member = command_handlers.run_add_world_room_member
run_remove_world_room_member = command_handlers.run_remove_world_room_member
run_rotate_world_room_invite = command_handlers.run_rotate_world_room_invite
run_join_world_room_by_invite = command_handlers.run_join_world_room_by_invite
run_set_sect_directive = command_handlers.run_set_sect_directive
run_clear_sect_directive = command_handlers.run_clear_sect_directive
run_intervene_sect_relation = command_handlers.run_intervene_sect_relation
...
def get_runtime_run_config() -> object:
    """兼容保留：获取当前运行配置。"""
    return _get_runtime_run_config(room_registry.get_runtime())
